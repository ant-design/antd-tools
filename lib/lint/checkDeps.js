...
function getModules(filePath, globConfig, debugAst) {
  ...
  const astBody = result.ast.program.body;

  if (debugAst) debug('>>>', JSON.stringify(astBody, null, 2));
...
module.exports = function(done) {
  try {
    ...
    const tsFiles = glob.sync(componentPath, globConfig)
      .filter(tsPath => (
        !/components\/_/.test(tsPath) &&
        !/~/.test(tsPath)
      ));
    ...
    tsFiles.forEach((filePath, index) => {
      ...
      const rootStyleModuleList = getModules(componentRootStylePath, {});
      const missStyleModuleList = moduleRootStyleList.filter((styleModulePath) => (
        rootStyleModuleList.every(module => !module.path.includes(styleModulePath))
      ));

      if (missStyleModuleList.length) {
        console.log(chalk.red(`[${id}]`, 'Dependency style file(s) not included in:', componentRootStylePath));
