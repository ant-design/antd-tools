...
module.exports = function(done) {
  ...
  try {
    ...
    tsFiles.forEach((filePath, index) => {
      ...
      const componentModuleList = componentModules[componentRoot] = componentModules[componentRoot] || [];
    ...
    Object.keys(componentModules).forEach((componentPath) => {
      ...
      if (missStyleModuleList.length || uselessStyleModuleList.length) {
        ...
        console.log(
          chalk.red(
            'Dependency style file(s) not included in:',
            componentRootStylePath
          )
        );
        ...
        uselessStyleModuleList.forEach(({ path: uselessPath }) => {
          console.log(
            chalk.yellow(' - [Useless]'),
            uselessPath
          );
        });

        console.log(
          `(debug id: ${componentIds[componentPath].join(', ')})`
        );
      } else {
