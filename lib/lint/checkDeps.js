...
const SKIP_REGEX = /deps-lint-skip:\s*([\w\d-_,]*)/;
...
function getModules(filePath, globConfig, debugAST) {
  ...
  const ignoreList = (astComments || []).reduce((list, { value }) => {
    const match = value.match(SKIP_REGEX);
    if (match) {
      return [...list, ...match[1].split(/,\s*/)];
    }
    return list;
...
module.exports = function(done) {
  ...
  try {
    ...
    tsFiles.forEach((filePath, index) => {
      ...
      const {
        moduleList: rootStyleModuleList, ignoreList: rootStyleModuleIgnoreList,
