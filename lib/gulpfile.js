...
gulp.task('start', ['clean'], (done) => {
  ...
  const plugins = `${webpackPlugin}?disableNpmInstall,
     ${argv.livereload === false ? '' : `${livereloadPlugin}?enableJs=false,`}
...
gulp.task('run_site', (done) => {
  runCmd('npm', ['run', 'site'], (code) => {
    done(code);
  });
});

gulp.task('just-deploy', ['check-git', packageJson.scripts.site ? 'run_site' : 'site'], (done) => {
...
function babelify(js) {
  return js.pipe(babel(babelConfig))
    ...
    .pipe(through2.obj(function (file, encoding, next) {
      ...
      if (file.path.match(/\/style\/index(\.web)?\.tsx$/)) {
