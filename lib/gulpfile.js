...
function babelify(js, modules) {
  ...
  return js.pipe(babel(babelConfig))
    ...
    .pipe(gulp.dest(modules ? 'es' : 'lib'));
}

function compile(modules) {
  modules ? execSync('rimraf lib es') : execSync('rimraf lib');
  const less = gulp.src(['components/**/*.less'])
    ...
    .pipe(gulp.dest(modules ? 'es' : 'lib'));
  const assets = gulp.src(['components/**/*.@(png|svg)']).pipe(gulp.dest(modules ? 'es' : 'lib'));
  let error = 0;
  const source = [
    'components/**/*.tsx',
    'typings/**/*.d.ts',
  ...
  const tsd = tsResult.dts.pipe(gulp.dest(modules ? 'es' : 'lib'));
...
function publish(tagString, done) {
  ...
  runCmd(publishNpm, args, (code) => {
    if (!code) {
      modules ? execSync('rimraf es dist') : execSync('rimraf lib dist');
    }
    tag();
    done(code);
  });
...
gulp.task('compile-with-es', ['compile'], () => {
  compile(true);
});
...
gulp.task('pub', ['check-git', 'compile-with-es'], (done) => {
  pub(done);
});
