...
gulp.task('ts-lint', gulp.series(done => {
...
gulp.task('ts-lint-fix', gulp.series(done => {
  const tslintBin = require.resolve('tslint/bin/tslint');
  const tslintConfig = path.join(__dirname, './tslint.json');
...
gulp.task('compile', gulp.series('compile-with-es', () => {
  compile();
}));

gulp.task('install', gulp.series(done => {
  install(done);
}));
...
gulp.task('update-self', gulp.series(done => {
  getNpm(npm => {
